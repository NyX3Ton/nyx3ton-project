...
def create_research_plans_table():
    ...
    cursor = conn.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS research_plans (id INTEGER PRIMARY KEY,short_summary TEXT NOT NULL,details TEXT NOT NULL)""")
    conn.commit()
    conn.close()
