...
def start_mlflow_ui(open_browser: bool = False) -> Optional[subprocess.Popen]:
    ...
    # When binding to 0.0.0.0 inside Docker, check localhost instead.
    port_check_host = "127.0.0.1" if MLFLOW_UI_HOST in {"0.0.0.0", "::"} else MLFLOW_UI_HOST

    if _is_port_open(port_check_host, MLFLOW_UI_PORT):
        public_host = "127.0.0.1" if MLFLOW_UI_HOST in {"0.0.0.0", "::"} else MLFLOW_UI_HOST
        print(f"MLflow UI already running at http://{public_host}:{MLFLOW_UI_PORT}")
        if open_browser:
            webbrowser.open(f"http://{public_host}:{MLFLOW_UI_PORT}")
        return None
    ...
    public_host = "127.0.0.1" if MLFLOW_UI_HOST in {"0.0.0.0", "::"} else MLFLOW_UI_HOST
    print(f"MLflow UI started at http://{public_host}:{MLFLOW_UI_PORT}")
...
def load_pdf(path: str) -> str:
    # PyMuPDF is installed as `PyMuPDF`; modern import is `pymupdf`,
    # while many examples still use the legacy `fitz` import. Support both.
    try:
...
if __name__ == "__main__":
    ...
    # In Docker, the app must listen on 0.0.0.0 inside the container.
    # On a local host run, the defaults keep it local-only.
    gradio_server_name = os.getenv("GRADIO_SERVER_NAME", "127.0.0.1")
    gradio_server_port = int(os.getenv("GRADIO_SERVER_PORT", "7860"))
    gradio_inbrowser = os.getenv("GRADIO_INBROWSER", "1").strip().lower() in {"1", "true", "yes", "y"}
