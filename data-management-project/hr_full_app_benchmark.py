...
def save_to_postgre(df: pd.DataFrame, table_name: str) -> None:
    engine = postgre_engine()
    schema = postgre_schema()
...
def evaluate_binary_model(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.50) -> dict[str, float]:
    ...
    for ax, (metric, title) in zip(axes, metrics):
        for label, grp in resource_samples_df.groupby("label"):
            if metric in grp.columns and grp[metric].notna().any():
                ax.plot(grp["seconds_from_start"], grp[metric], label=label)
        ax.set_title(title)
...
def predict_tabm_proba(model: TabMClassifier, x_num: np.ndarray, x_cat: np.ndarray, batch_size: int = 4096) -> np.ndarray:
    ...
    for start in range(0, len(x_num), batch_size):
        ...
        probs = torch.sigmoid(logits).mean(dim=1)       # priemerujeme pravdepodobnosti
...
def make_tabm_config_from_trial(trial: optuna.Trial) -> TabMConfig:
    ...
    return TabMConfig(
                        ...
                        d_block=trial.suggest_int("d_block", 128, 768, step=64),
...
def train_tabm(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for xb_num, xb_cat, yb in train_loader:
            ...
            optimizer.zero_grad()
            logits = model(xb_num, xb_cat).squeeze(-1)              # (B, k)
            y_rep = yb.unsqueeze(1).expand_as(logits)               # (B, k)
...
def main() -> None:
    ...
    xgb_metrics = evaluate_binary_model(y_test.to_numpy(), xgb_test_prob, threshold=XGB_THRESHOLD)
    
    print_metric_block("XGBoost", xgb_metrics)
    ...
    tabm_metrics = evaluate_binary_model(y_test.to_numpy(), tabm_test_prob, threshold=TABM_THRESHOLD)
    print_metric_block("TabM", tabm_metrics)
    ...
    joblib.dump(
                {
                    ...
                    "cat_cardinalities": preprocessor.categorical_cardinalities,
                    "config": asdict(best_tabm_config),
                },
                MODEL_DIR / "tabm_comparison_metadata.pkl",)

    tabm_predictions_df = build_prediction_output(
                                                    meta_df=meta_test,
                                                    y_true=y_test.to_numpy(),
                                                    y_prob=tabm_test_prob,
                                                    threshold=TABM_THRESHOLD,
                                                    model_name="TabM",
                                                )

    tabm_history_df.to_csv(OUTPUT_DIR / "tabm_training_history.csv", index=False, encoding="utf-8-sig")
    plot_tabm_training_curves(tabm_history_df, PLOTS_DIR / "tabm_training_curves.png")

    # -----------------------------------------------------
    # f. Final comparison / export
    # -----------------------------------------------------
    comparison_df = pd.DataFrame([{"Model": "XGBoost", **xgb_metrics},{"Model": "TabM", **tabm_metrics},]).sort_values(by="roc_auc", ascending=False)
    ...
    if USE_SQL_EXPORT:
        save_to_postgre(comparison_df, "Model_Comparison_TabM_vs_XGBoost")
        save_to_postgre(xgb_predictions_df, "Predictions_Attrition_XGBoost")
        save_to_postgre(tabm_predictions_df, "Predictions_Attrition_TabM")
        save_to_postgre(xgb_importance, "Model_Feature_Importance_XGBoost")
        save_to_postgre(resource_summary_df, "Benchmark_Resource_Summary")
        save_to_postgre(resource_samples_df, "Benchmark_Resource_Samples")
        save_to_postgre(tabm_history_df, "Benchmark_TabM_Training_History")
